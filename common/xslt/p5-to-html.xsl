...
<xsl:stylesheet 
          ...
          <div>
		    ...
		    -->
              <xsl:for-each select="fileDesc/titleStmt/author">
                <!-- Declare the variable for the persName value at the author level -->
                <xsl:variable name="persNameValue">
                  <xsl:call-template name="lookup-persName-by-id">
                    <xsl:with-param name="ref" select="persName/@ref"/>
                  </xsl:call-template>
                </xsl:variable>
                <!-- Output the persName value -->
                <xsl:value-of select="$persNameValue"/>
                <!-- For the last author, check the ending punctuation -->
                <xsl:if test="position() = last()">
                  <xsl:choose>
                    <xsl:when test="substring($persNameValue, string-length($persNameValue), 1) = '.'">
                      <xsl:text> </xsl:text>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:text>. </xsl:text>
  ...
  <xsl:template mode="create-attributes" match="*">
    <xsl:attribute name="class" select="         string-join((concat('tei-', local-name()), for $corresp in tokenize(@corresp)  return  concat('corresp-', substring-after($corresp, '#')),  for $ref in tokenize(@ref)  return  concat('ref-', substring-after($ref, '#')),      for $type in tokenize(@type)         return           concat('type-', $type),         for $place in tokenize(@place)         return           concat('place-', $place),         for $role in tokenize(@role)  return    concat('role-', $role),         for $resp in tokenize(@resp)         return           concat('resp-', substring-after($resp, '#')),         for $rendition in tokenize(if (@rendition) then           @rendition         else           $tag-usage[@gi = local-name(current())]/@rendition)         return           concat('rendition-', substring-after($rendition, '#')),         if (@hand) then           'hand'         else           ()), ' ')"/>
  ...
  <xsl:template mode="citation-popup" match="title[@type = 'short']"><cite><xsl:value-of select="."/></cite>. </xsl:template>
  ...
  <xsl:template match="figure[@type = 'map']">
    ...
    <div class="map">
      <iframe id="funeral-map" sandbox="allow-scripts allow-same-origin allow-popups" style="border:1px solid black;" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://www.google.com/maps/embed/v1/view?key={$google-api-key}&amp;center={$latitude},{$longitude}&amp;zoom=18&amp;maptype=satellite"/>
