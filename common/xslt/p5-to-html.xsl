...
<xsl:stylesheet 
  ...
  <xsl:template mode="create-attributes" match="*">
    <xsl:attribute name="class" select="         string-join((concat('tei-', local-name()), for $corresp in tokenize(@corresp)  return  concat('corresp-', substring-after($corresp, '#')),        for $type in tokenize(@type)         return           concat('type-', $type),         for $place in tokenize(@place)         return           concat('place-', $place),         for $role in tokenize(@role)  return    concat('role-', $role),         for $resp in tokenize(@resp)         return           concat('resp-', substring-after($resp, '#')),         for $rendition in tokenize(if (@rendition) then           @rendition         else           $tag-usage[@gi = local-name(current())]/@rendition)         return           concat('rendition-', substring-after($rendition, '#')),         if (@hand) then           'hand'         else           ()), ' ')"/>
