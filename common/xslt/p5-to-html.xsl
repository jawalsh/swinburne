...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:swinburne="tag:biblicon.org,2024:swinburne" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://www.w3.org/1999/xhtml" version="3.0" xpath-default-namespace="http://www.tei-c.org/ns/1.0">
  ...
  <xsl:template mode="create-attributes" match="*">
    <xsl:attribute name="class" select="         string-join((concat('tei-', local-name()),         for $type in tokenize(@type)         return           concat('type-', $type),         for $place in tokenize(@place)         return           concat('place-', $place),         for $role in tokenize(@role)  return    concat('role-', $role),         for $resp in tokenize(@resp)         return           concat('resp-', substring-after($resp, '#')),         for $rendition in tokenize(if (@rendition) then           @rendition         else           $tag-usage[@gi = local-name(current())]/@rendition)         return           concat('rendition-', substring-after($rendition, '#')),         if (@hand) then           'hand'         else           ()), ' ')"/>
